...
def main():
    """
    Main CLI entry point for training the FTW model.
    Mirrors the behavior of `ftw model fit`.
    """
    # LightningCLI handles argument parsing from YAML and CLI flags
    cli = LightningCLI(
        model_class=CustomSemanticSegmentationTask,
        datamodule_class=FTWDataModule,
        save_config_kwargs={"overwrite": True},
        run=False,  # prevents auto-fitting; we call fit manually
    )
