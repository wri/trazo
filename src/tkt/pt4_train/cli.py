...
def parse_args():
    parser = argparse.ArgumentParser(description="FTW model training CLI")
    parser.add_argument("--config", required=True, help="Path to YAML config")
    parser.add_argument("--data-dir", default=None, help="Override dataset root")
    parser.add_argument("--output-dir", default=None, help="Override output path")
...
def main():
    ...
    # Override output / checkpoint dirs
    if args.output_dir:
        config["trainer"]["default_root_dir"] = args.output_dir

        # Update checkpoint callback dir
        for cb in config["trainer"].get("callbacks", []):
            if cb.get("class_path") == "lightning.pytorch.callbacks.ModelCheckpoint":
                cb["init_args"]["dirpath"] = os.path.join(args.output_dir, "checkpoints")

        # Update logger dir
        for lg in config["trainer"].get("logger", []):
            if lg.get("class_path") == "lightning.pytorch.loggers.CSVLogger":
                lg["init_args"]["save_dir"] = os.path.join(args.output_dir, "metrics")
    ...
    # Prepare CLI args for LightningCLI
    cli_args = [args.subcommand, f"--config={tmp_config_path}"]
    if args.ckpt:
        cli_args += [f"--ckpt_path={args.ckpt}"]
